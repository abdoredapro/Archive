            ...
            {{-- <form action="{{ route('dashboard.file.store') }}" method="POST" enctype="multipart/form-data"> --}}
                ...
                <div class="sec-white-board create-file">
                    <div class="stack">
                        ...
                        <div class="text-module">
                            <div class="one-content">
                                <h3 class="text-end">اسم المقطع</h3>
                                <input type="text" id="movie-name" name="name" placeholder="اسم المقطع" value="{{ old('name') }}" style="border: 1px solid #ccc" required>
                    ...
                    <div class="modal-footer">
                        <button class="confirm" type="submit" id="save">حفظ الملف</button>
                    </div>
...
<script>
    ...
    saveBtn.onclick = function () {
        ...
        var r = new Resumable({
                ...
                target: '{{ route('dashboard.file_clip.store',$id) }}',
            ...
            r.on('fileAdded', function(file, event){
                r.upload();
                toastr.info('جارى تحميل المقطع');
            });

            r.on('fileSuccess', function(file, message){
                toastr.success('تم تحميل المقطع');
                window.location.href = '{{ route('dashboard.file.show', $id) }}';
            });

            r.on('fileError', function(file, message){
                toastr.error('حدث خطأ الرجاء المحاوله مره اخرى');
            });
    }
